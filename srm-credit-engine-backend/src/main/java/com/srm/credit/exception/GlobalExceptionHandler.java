...
public class GlobalExceptionHandler {
    ...
    public ProblemDetail handleDataIntegrity(DataIntegrityViolationException ex) {
        log.warn("Database integrity violation intercepted. Possible concurrent duplicate request: {}", ex.getMessage());

        ProblemDetail pd = ProblemDetail.forStatusAndDetail(
                HttpStatus.CONFLICT,
                "A duplicate transaction with the same values was detected. Please wait a minute before retrying."
        );
        pd.setTitle("Duplicate request blocked");
        return pd;
    }
}
